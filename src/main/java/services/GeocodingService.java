...
public class GeocodingService {
    ...
    // Cache local pour éviter les appels réseau répétés
    private final Map<String, double[]> cache = new LinkedHashMap<>();

    /**
     * Géocode une localisation textuelle en coordonnées [lat, lon].
     * Ajoute ", Tunisie" si aucun pays n'est spécifié pour améliorer la précision.
     *
     * @param location ex: "Tunis", "Ariana", "Sousse"
     * @return double[] {latitude, longitude} ou null si introuvable
     */
    // Compteur pour respecter la limite Nominatim (1 req/sec)
    ...
    public double[] geocode(String location) {
        ...
        try {
            ...
            // Ajouter le pays par défaut si absent
            ...
            if (responseCode != 200) {
                System.err.println("Nominatim HTTP " + responseCode + " pour '" + query + "'");
                return null;
            }
            ...
            if (lat != 0 || lon != 0) {
                ...
                System.out.println("Géocodé : " + location + " → [" + lat + ", " + lon + "]");
                return coords;
            }

        } catch (Exception e) {
            System.err.println("Erreur géocodage pour '" + location + "' : " + e.getMessage());
        }
    ...
    /** Extraction minimaliste d'un double depuis un JSON brut */
    private double extractJsonDouble(String json, String key) {
        // Vérifier que le JSON n'est pas un tableau vide
        ...
        // Chercher le début du nombre (skip quotes/spaces)

...
public class CurrencyService {
    ...
    public enum Devise {
        ...
        public String toString() {
            return symbole + "  " + label + " (" + code + ")";
        }
    }

    /**
     * Resultat d'une conversion de salaire.
     */
    public static class ConversionResult {
        ...
        /**
         * Genere un texte lisible avec toutes les conversions.
         */
        public String toDisplayText() {
            ...
            sb.append("  ").append(deviseSource.getCode()).append(" : ")
              ...
              .append(fmt(salaireMaxOriginal)).append(" ").append(deviseSource.getSymbole())
              .append("\n\n");

            if (conversions.isEmpty()) {
                sb.append("Aucune conversion disponible.\n");
                sb.append("Verifiez votre connexion internet.\n");
                return sb.toString();
            }
            ...
            for (var entry : conversions.entrySet()) {
                ...
                sb.append("  ").append(d.getCode()).append(" : ")
                  ...
                  .append(fmt(vals[1])).append(" ").append(d.getSymbole())
                  .append("\n");
            }
        ...
        /** Formate un montant : 2 decimales, separateur milliers point */
    ...
    // Cache des taux (evite les appels reseau repetes pendant la session)
    ...
    private static final long CACHE_DURATION_MS = 10L * 60L * 1000L; // 10 minutes

    /**
     * Convertit une fourchette salariale depuis une devise source vers toutes les autres devises.
     */
    public ConversionResult convertir(double salaireMin, double salaireMax, Devise deviseSource) {
        ...
        try {
            ...
            if (rates == null || rates.isEmpty()) {
                result.setErrorMessage("Impossible de recuperer les taux de change. Verifiez votre connexion internet.");
                return result;
            }
            ...
            System.err.println("[CurrencyService] Erreur de conversion : " + e.getMessage());
    ...
    /**
     * Recupere les taux de change pour une devise de base (avec cache).
     */
    private Map<String, Double> fetchRates(String baseCurrency) {
        long now = System.currentTimeMillis();

        // Utiliser le cache si pas expire
        ...
        try {
            ...
            if (responseCode == 200) {
                ...
                System.err.println("[CurrencyService] HTTP " + responseCode);
            }
        ...
        // Retourner le cache meme expire plutot que null
        return ratesCache.get(baseCurrency);
    }

    /**
     * Parse le JSON de la reponse pour extraire les taux.
     * Format : {"result":"success",...,"rates":{"TND":1,"EUR":0.28,"USD":0.31,...}}
     */
        ...
        try {
            ...
            for (Devise d : Devise.values()) {
                ...
                if (idx != -1) {
                    ...
                    try {
                        rates.put(d.getCode(), Double.parseDouble(valueStr));
                    } catch (NumberFormatException e) {
                        System.err.println("[CurrencyService] Parse error : " + d.getCode() + " = '" + valueStr + "'");
                    }
                }
            }

        } catch (Exception e) {
            System.err.println("[CurrencyService] Erreur parsing JSON : " + e.getMessage());
            return null;
        }
    ...
    /**
     * Extrait la date de derniere mise a jour du JSON.
     */

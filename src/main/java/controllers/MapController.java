 ...
 * Affiche une carte Leaflet.js dans un WebView integre a l'application.
 * Geocodage via Nominatim, calcul de distance Haversine, filtre par rayon.
 */
public class MapController {
    ...
    /** Appele pour une offre specifique (depuis le bouton Carte) */
    ...
    /** Charge les offres, geocode, genere le HTML et l'affiche dans le WebView */
    private void chargerCarte() {
        lblMapInfo.setText("Chargement des offres et geocodage...");
        ...
        Task<String> task = new Task<>() {
            ...
            protected String call() throws Exception {
                ...
                for (Offre o : toutesLesOffres) {
                    ...
                    if (coords != null) {
                        markers.add(new MarkerData(o, coords[0], coords[1]));
                    }
                }

                double centerLat = 34.0;
                double centerLon = 9.0;
                int zoom = 7;
        ...
        task.setOnSucceeded(e -> {
            ...
            lblMapInfo.setText(toutesLesOffres.size() + " offres chargees - carte affichee");
        });
    ...
    /** Filtre par rayon depuis une position donnee */
    @FXML
    private void handleFiltrerRayon() {
        ...
        lblMapInfo.setText("Geocodage de " + position + "...");
        ...
        Task<String> task = new Task<>() {
            ...
            protected String call() throws Exception {
                ...
                if (origin == null) {
                    throw new Exception("Impossible de localiser : " + position);
                }
        ...
        task.setOnSucceeded(e -> {
            ...
            lblMapInfo.setText("Carte mise a jour");
        });
    ...
    private void retourOffres() {
        try {
            ...
            Stage stage = (Stage) lblMapInfo.getScene().getWindow();
    ...
    private String buildHtmlWithOrigin(List<MarkerData> markers, double originLat, double originLon, double maxKm, int count) {
        ...
        String extra = "<div style='position:fixed;bottom:10px;left:10px;background:white;padding:8px 14px;"
                ...
                + (maxKm < Double.MAX_VALUE ? String.format(Locale.US, "%.0f km", maxKm) : "illimite") + "</b></div>";
    ...
    private double parseRayon(String str) {
        ...
        try {
            return Double.parseDouble(str.replace("km", "").trim());
